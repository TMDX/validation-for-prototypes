...
function checkDropDown(errors) {
    $(document).find('select').each(function () {
        var $formgroup = $(this).parents('fieldset');
        var label = $formgroup.find('label').clone().children().remove().end().text();

        if ($formgroup.attr('data-required') !== undefined && $(this).val() === '') {
            if ($(this).attr('id') === undefined) {
                $(this).attr('id', $(this).attr('name'));
            }
    ...
    for (var i = 0; i < errors.length; i++) {
        if ($(document).find('a[href="#' + errors[i].id + '"]').length === 0) {
            ...
            if ($formgroup.find('.error-message').length === 0) {
                if ($formgroup.find('input[type="text"], input[type="password"], select').length > 0 || $formgroup.find('textarea').length > 0) {
